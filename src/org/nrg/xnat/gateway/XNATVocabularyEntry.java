...
*/
	...
	{
		int group=Integer.parseInt(el.attributeValue("dicom_group"),16),
			elem =Integer.parseInt(el.attributeValue("dicom_element"),16);
...
//		m_header=el.attributeValue("xnat_header");
		...
		if(llsw.size()>0) m_criteria = llsw;
	}
	public void toElement(Element el)
	{
		el.addAttribute("dicom_group", Integer.toHexString(getGroup(m_DICOMTag)));
		el.addAttribute("dicom_element", Integer.toHexString(getElem(m_DICOMTag)));
...
//		el.addAttribute("xnat_header", m_header);
	...
	{
		if(m_criteria==null) return null;
