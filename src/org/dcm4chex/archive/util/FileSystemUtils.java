...
public class FileSystemUtils
{
	...
	static
	{
		try
		{
			jdk6getUsableSpace = File.class.getMethod("getUsableSpace",
					(Class[]) null);
		} catch (Exception e)
		{
	...
	public static long freeSpace(String path) throws IOException
	{
		if (jdk6getUsableSpace != null)
		{
			try
			{
				Long l = (Long) jdk6getUsableSpace.invoke(new File(path),
						(Object[]) null);
	...
	long freeSpaceWindows(String path) throws IOException
	{
		...
		String[] cmdAttrbs = new String[]{"cmd.exe", "/C",
				"dir /c \"" + path + "\""};
		...
		outerLoop : while (i > 0)
		{
			...
			if (line.length() > 0)
			{
				...
				{// Loop until found two adjoining non digit characters.
					char c = line.charAt(j);
					if (!Character.isDigit(c)
							&& (j == 0 || !Character
