 ...
 * @version $Revision: 6312 $ $Date: 2008-05-15 15:06:29 +0200 (Thu, 15 May
 *          2008) $
 * @since 31.08.2003
 */
...
*/
			...
			AEDTO ae, FileInfo[][] fileInfos) throws Exception
	{
		 ...
		 * !service.hasUnrestrictedExportPermissions(a.getCallingAET()); boolean
		 * checkReadPermissions =
		 * !service.hasUnrestrictedReadPermissions(ae.getTitle()); if
		 * (!checkExportPermissions && !checkReadPermissions) { return; }
		 * HashSet suids = new HashSet(); for (int i = 0; i < fileInfos.length;
		 * i++) { suids.add(fileInfos[i][0].studyIUID); } Subject subject =
		 * (Subject) a.getProperty("user"); StudyPermissionManager
		 * studyPermissionManager = service.getStudyPermissionManager(a); if
		 * (checkExportPermissions) { if (subject == null) { throw new
		 * DcmServiceException( MISSING_USER_ID_OF_MOVE_SCU_ERR_STATUS,
		 * MISSING_USER_ID_OF_MOVE_SCU_ERR_MSG); } for (Iterator iter =
		 * suids.iterator(); iter.hasNext();) { if
		 * (!studyPermissionManager.hasPermission((String) iter.next(),
		 * StudyPermissionDTO.EXPORT_ACTION, subject)) { throw new
		 * DcmServiceException( NO_EXPORT_PERMISSION_ERR_STATUS,
		 * NO_EXPORT_PERMISSION_ERR_MSG); } } } if (checkReadPermissions) { if
		 * (subject == null || thirdPartyMove) { subject = new Subject(); String
		 * userId = ae.getUserID(); String passwd = ae.getPassword(); if (userId
		 * == null || userId.length() == 0 ||
		 * !service.dicomSecurity().isValid(userId, passwd, subject)) { throw
		 * new DcmServiceException( MISSING_USER_ID_OF_STORE_SCP_ERR_STATUS,
		 * MISSING_USER_ID_OF_STORE_SCP_ERR_MSG); } } for (Iterator iter =
		 * suids.iterator(); iter.hasNext();) { if
		 * (!studyPermissionManager.hasPermission((String) iter.next(),
