 ...
 * @version $Revision: 12829 $ $Date: 2010-02-26 14:29:17 +0100 (Fri, 26 Feb
 *          2010) $
 * @since 22.05.2004
 * 
 */
public class DecompressCmd extends CodecCmd
{
	...
	public static byte[] decompressFile(File inFile, File outFile,
			String outTS, int planarConfiguration, int pxdataVR, byte[] buffer)
			throws Exception
	{
		...
		try
		{
			...
			try
			{
				...
				if (planarConfiguration >= 0
						&& ds.contains(Tags.PlanarConfiguration))
	...
	{
		String pmi = ds
				.getString(Tags.PhotometricInterpretation, "MONOCHROME2");
		if (pmi.startsWith("YBR")
				&& (tsOrig.equals(UIDs.JPEGBaseline)
						|| tsOrig.equals(UIDs.JPEGExtended)
						|| tsOrig.equals(UIDs.JPEG2000Lossless) || tsOrig
						.equals(UIDs.JPEG2000Lossy)))
		{
			...
			throws Exception
	{
		...
		try
		{
			...
			{
				int frame = simpleFrameList != null
						? (simpleFrameList[i] - 1)
						: i;
			...
			ByteOrder byteOrder) throws IOException
	{
		...
		ComponentSampleModel sm = (ComponentSampleModel) raster
				.getSampleModel();
		checkSampleModel(sm);
		...
		for (int b = 0; b < numBanks; b++)
		{
			switch (buffer.getDataType())
			{
				case DataBuffer.TYPE_BYTE :
					for (int i = 0; i < h; ++i)
						out.write(((DataBufferByte) buffer).getData(b), i
								* stride, l);
					break;
				case DataBuffer.TYPE_USHORT :
					if (byteOrder == ByteOrder.LITTLE_ENDIAN)
						for (int i = 0; i < h; ++i)
							writeShortLE(
									((DataBufferUShort) buffer).getData(b), i
											* stride, l, out);
					else
						for (int i = 0; i < h; ++i)
							writeShortBE(
	...
	private void checkSampleModel(ComponentSampleModel sm)
	{
		...
		if (!(Arrays.equals(OFF_0, bandOffsets) && Arrays.equals(OFF_0,
				bankIndices))
				&& !(Arrays.equals(OFF_0_0_0, bandOffsets) && Arrays.equals(
						OFF_0_1_2, bankIndices))
				&& !(Arrays.equals(OFF_0_0_0, bankIndices) && Arrays.equals(
						OFF_0_1_2, bandOffsets)))
