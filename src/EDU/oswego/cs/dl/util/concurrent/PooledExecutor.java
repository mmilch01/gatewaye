...
public class PooledExecutor extends ThreadFactoryUser implements Executor
{
	 ...
	 *                if the current thread has been interrupted in the course
	 *                of waiting
	 */
	...
	protected Runnable getTask() throws InterruptedException
	{
		...
		synchronized (this)
		{
			if (poolSize_ > maximumPoolSize_) // Cause to die if too many
												// threads
				return null;
		...
		for (;;)
		{
			synchronized (this)
			{
				if (!shutdown_)
				{
					...
					// If cannot handoff and still under maximum, create new
					// thread
